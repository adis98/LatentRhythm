...
def generate(output_directory,
    ...
    testing_data = torch.from_numpy(testing_data).float().cuda() if gpu_available else torch.from_numpy(testing_data).float()
    print('Data loaded')
    ...
    for i, batch in enumerate(testing_data):
        ...
        if masking == 'mnr':
            ...
            mask = mask.type(torch.float).cuda() if gpu_available else mask.type(torch.float)
            ...
            mask = mask_T.permute(1, 0)
            mask = mask.repeat(batch.size()[0], 1, 1).float().cuda() if gpu_available else mask.repeat(batch.size()[0], 1, 1).float()
