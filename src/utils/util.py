...
def std_normal(size):
    ...
    return torch.normal(0, 1, size=size).cuda() if torch.cuda.is_available() else torch.normal(0, 1, size=size)
...
def training_loss(net, loss_fn, X, diffusion_hyperparams, only_generate_missing=1):
    ...
    diffusion_steps = torch.randint(T, size=(B, 1, 1)).cuda()  if torch.cuda.is_available() else torch.randint(T, size=(B, 1, 1)) # randomly sample diffusion steps from 1~T
